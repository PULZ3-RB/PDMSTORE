export async function onRequestPost(context) {
  try {
    ...
    if (!webhook) {
      return jsonResponse({
        ok: false,
        error: "Webhook de Discord no configurado en Cloudflare."
      }, 500);
    }
    ...
    const vehiculo = clean(data.vehiculo);
    const precio = clean(data.precio);

    if (!nombre || !apellidos || !telefono || !vehiculo || !precio) {
      return jsonResponse({
        ok: false,
      ...
      embeds: [
        {
          ...
          footer: {
            text: "PDMSTORE · Nueva solicitud de reserva"
          },
    ...
    if (!discordResponse.ok) {
      ...
      console.error("Discord:", errorText);
